...
RSpec.describe ReviewsController, type: :controller do
    ...
    let(:place) { create(:place, user: user, address: '東京都千代田区丸の内一丁目９番１号', activity_type: '公園', name: '東京駅') }
    let!(:review) { create(:review, user: user, place: place, body: '素晴らしい場所です', rating: 5) }
    ...
    describe 'GET #index' do
        ...
        context 'フィルタ機能' do
            let!(:place1) { create(:place, user: user, address: '東京', activity_type: '公園', name: '美しい公園') }
            let!(:place2) { create(:place, user: user, address: '大阪', activity_type: '室内遊び場', name: '面白い室内遊び場') }
            let!(:place3) { create(:place, user: user, address: '京都', activity_type: '公園', name: '静かな公園') }
            let!(:review1) { create(:review, user: user, place: place1, body: '美しい公園のレビュー', rating: 4) }
            let!(:review2) { create(:review, user: user, place: place2, body: '面白い室内遊び場のレビュー', rating: 3) }
            let!(:review3) { create(:review, user: user, place: place3, body: '静かな公園のレビュー', rating: 5) }
